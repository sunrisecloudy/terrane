...
- **CR-12** **Cross-engine conformance suite** (release-blocking): every host API, limit behavior, and deterministic-replay case tested on QuickJS-native, QuickJS-WASM, and JSC in CI. Built in M0b on top of the M0a spine; this is the heart of the platform template. The central acceptance test of the whole product is the spine itself: *TS → SWC → QuickJS-WASM → Rust capability `ctx` → SQLite write → UI tree patch → deterministic replay, fully offline.*
...
- Conformance suite green on all three engine targets; any divergence blocks release.
