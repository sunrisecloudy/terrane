...
| I2 (DL-17) | `transact([...])` collection scope in M0a | **A `transact` is scoped to a SINGLE collection in M0a: a group spanning more than one distinct collection is REJECTED with a typed `QueryError` ("transact must target a single collection"). The rejection is enforced (and recorded for replay) at the runtime `ctx.db.transact` host call BEFORE any state changes, with the storage bridge rejecting the same group as defense-in-depth. Multi-collection atomic ACROSS THE SYNC BOUNDARY (transaction-group metadata + all-or-nothing apply at the receiver) is a future DL-17 extension.** | A multi-collection group is locally atomic, but it persists one CRDT chunk/oplog row PER collection with no transaction-group id, and `forge-sync` authorizes/applies each chunk independently across the SS-7 boundary (reviews 129/131/132). A peer denied one collection would import a torn HALF of the group — not sync-safe. Scoping M0a `transact` to one collection makes "atomic locally, merged as a unit" honest under SS-7 today; the cross-collection version needs transaction-group framing the protocol does not yet carry. The runtime preflight records the rejection through the denial/error-recording path (db.watch review 137) so a failed run captures the `db.transact` event and replays to the SAME error byte-identically, instead of the bridge raising it unrecorded inside the live closure (replay divergence). |
| I3 (SC-10) | Which SC-10 gates apply at the remote-sync boundary (SS-7) | **SC-10's full seven-gate decision applies on every COMMAND/RUN. A REMOTE SYNC OP evaluates only the SYNC-APPLICABLE gates: the SS-7 RBAC role/membership decision AND the workspace-policy capability gate, applied per the chunk's capability CATEGORY (a record write → `Db`; a DL-13 migration chunk's schema authority → the membership `schema_write` RBAC grant). The run-profile, platform-permission, manifest, resource-allowlist, and rate/resource-limit gates are RUN-scoped and do NOT apply to a chunk import. A receiver whose workspace policy forbids a category skips an incoming chunk of THAT category even when membership would allow it.** (Normative in `07-security-prd.md` SC-10; `forge/spec/policy-gates.md` matches; wired in `forge/crates/core/src/workspace.rs` `authorize_incoming_op` / `sync_op_policy_category`.) | A passive remote CRDT chunk import is NOT a run: it executes no applet code, so it has no run profile, requests no manifest capability, touches no resource-allowlist scope, consumes no host-call budget, and uses no OS platform permission. Those five gates are properties of *executing* a run on this host and are meaningless for importing a peer's already-authored op — evaluating them would either fail-closed-deny every sync (no run profile/manifest exists for a peer's op) or be vacuous. The two gates that DO carry meaning at the boundary are who may write (SS-7 RBAC role/membership + per-collection grant; `schema_write` for a migration) and whether the workspace admin permits the capability category at all (workspace-policy gate 2). Earlier wording (review 164) hardcoded the workspace-policy gate to `Db`; review 165 generalizes it to the chunk's category so a receiver denying any category skips a chunk of that category, and makes the carve-out normative so code and PRD agree. |
