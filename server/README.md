...
- `GET /webapps/examples`
- `POST /control/command` for token-gated package validation/signing/install/rollback, app registry queries, `platform.health`, `runtime.capabilities`, and safe `db.*` inspection commands
...
Remaining persistence work: browser-backed smoke-test execution, migrations, data rollback/snapshot restore paths, and the Postgres production adapter.
