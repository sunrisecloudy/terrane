...
fn install_copies_the_bundle_into_the_home_and_runs_from_anywhere() {
    ...
    assert!(home.join("apps/todo-cli-collaborate/manifest.json").exists());
    let (_, state, _) = host(home, elsewhere, &["state"]);
    assert!(
        state.contains(&format!("{}", home.join("apps/todo-cli-collaborate").display())),
        "catalog should point into the home: {state}"
    );

    // Run it from an UNRELATED cwd — works because the home owns the bundle.
    let (ok, out, err) = host(home, elsewhere, &["run", "todo-cli-collaborate", "add", "buy milk"]);
    ...
    let (_, out, _) = host(home, elsewhere, &["run", "todo-cli-collaborate", "__actions__"]);
    assert!(out.contains("\"add\"") && out.contains("\"done\""), "actions: {out}");
