...
function checkServerStatic() {
  const serverLib = fs.readFileSync(path.join(repoRoot, "forge", "crates", "server", "src", "lib.rs"), "utf8");
  const serverMain = fs.readFileSync(path.join(repoRoot, "forge", "crates", "server", "src", "main.rs"), "utf8");
  const serverCargo = fs.readFileSync(path.join(repoRoot, "forge", "crates", "server", "Cargo.toml"), "utf8");
  const serverBuildTest = fs.readFileSync(path.join(repoRoot, "tools", "reference-host", "test", "forge-server-build.test.js"), "utf8");
  ...
  for (const snippet of [
    ...
    if (!serverLib.includes(snippet)) {
      throw new Error(`forge server lib missing ${snippet}`);
    }
  }
  for (const snippet of ["--bind", "--workspace", "--workspace-id", "serve_blocking", "ForgeServer::open", "ForgeServer::in_memory"]) {
    if (!serverMain.includes(snippet)) {
      throw new Error(`forge server main missing ${snippet}`);
    }
  }
  for (const snippet of ["name = \"forge-server\"", "forge-core.workspace", "forge-domain.workspace", "serde_json.workspace"]) {
    if (!serverCargo.includes(snippet)) {
      throw new Error(`forge server Cargo.toml missing ${snippet}`);
    }
  }
  for (const snippet of ["cargo", "test", "-p", "forge-server", "build", "--release", "--locked", "target", "release"]) {
    if (!serverBuildTest.includes(snippet)) {
      throw new Error(`forge server build test missing ${snippet}`);
    }
  }
  return "forge-server=health,bridge,events-drain,core-command,json-http,std-listener,cli-build-test";
}
...
function checkNativeStatic() {
  ...
  for (const snippet of ["SELECT response_json, url_pattern FROM network_mocks", "mockedNetworkResponse", "urlMatches", "delayMs"]) {
    if (!iosNetwork.includes(snippet)) {
      throw new Error(`iOS network mock source missing ${snippet}`);
    }
  }
  ...
  for (const snippet of ["SELECT response_json FROM dialog_mocks", "storedDialogMock"]) {
    if (!iosDialogs.includes(snippet)) {
      throw new Error(`iOS dialog mock source missing ${snippet}`);
    }
  }
  ...
  for (const snippet of [
    ...
    "Windows dev control supports static accessibility controls over bundled app HTML",
    "Windows dev control supports static smoke, micro-test, and platform smoke runners",
    "Windows dev control routes app registry lifecycle commands through fixed SQLite tables",
    "Windows dev control routes migration dry-run and apply through fixed SQLite helpers",
    "Windows dev control supports DB-backed network and dialog mocks",
    "Windows dev control supports direct storage get, set, and assertions",
    "Windows dev control exports and imports portable backups through fixed DB tables",
    "Windows dev control exports debug bundles through backup_exports",
    "X-Platform-Control-Token",
    "control_auth_required",
    "platform.health",
    "TERRANE_WINDOWS_CONTROL_READY port=",
