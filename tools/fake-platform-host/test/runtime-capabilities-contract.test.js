...
import { fakeHostCapabilities } from "../src/capabilities.js";
...
test("fake-host runtime.capabilities bridge response is app-scoped and schema-shaped", async () => {
  ...
  try {
    ...
    assertRuntimeCapabilitiesShape(response.result, "fake-host bridge response");
    assert.equal(response.result.appId, appId);
    assert.deepEqual(response.result, fakeHostCapabilities({ appId }));
  } finally {
    db.close();
  }
});
