...
export class ReferenceHost {
  ...
  verifyInstalledApp(appId) {
    ...
    if (!compatibility.ok && !this.allowRuntimeMismatch) {
      throw new PlatformError("runtime_version_incompatible", "App runtimeVersion is not compatible with the reference-host runtime", compatibility);
    }
    const missingCapabilities = missingRequiredCapabilities(installed.manifest, this.capabilities(appId));
    if (missingCapabilities.length > 0) {
      throw new PlatformError("capability_unavailable", "Required runtime capability is unavailable on reference-host", {
        appId,
        missingCapabilities,
      });
    }
  ...
  async runRepairLoop(args = {}) {
    ...
    for (let index = 0; index < maxAttempts; index += 1) {
      ...
      steps.push(repairStep("runtime.capabilities", "passed", referenceHostCapabilities(appId)));
  ...
  async runControlCommand(tool, args = {}) {
    ...
    switch (tool) {
      ...
      case "platform.list_targets":
        return {
          targets: [
            { id: "reference-host", platform: "reference-host", status: "available", runtimeVersion: this.runtimeVersion },
            { id: "macos", platform: "macos", status: "not-attached" },
            { id: "server", platform: "server", status: "not-attached" },
          ],
        };
      case "platform.launch":
        return { ok: true, target: args.target ?? "reference-host", status: "running", url: `http://127.0.0.1:${args.port ?? 7878}` };
      case "platform.stop":
        return { ok: true, target: args.target ?? "reference-host", status: "stopped" };
      ...
      case "platform.run_platform_smoke":
        return this.testRunner.runPlatformSmokeTest({ spec: args.spec, smokePath: args.smokePath, platform: args.platform ?? "reference-host" });
      ...
      case "db.export_backup":
        return this.database.exportBackup({ type: "backup", runtimeCapabilities: referenceHostCapabilities() });
      case "db.export_debug_bundle":
        return this.database.exportBackup({ type: "debug-bundle", runtimeCapabilities: referenceHostCapabilities(), includeDebug: true });
    ...
    try {
      if (req.method === "POST" && route.kind === "collection") {
        ...
        const session = this.database.createControlSession({
          target: body.target ?? "reference-host",
      ...
      if (req.method === "GET" && route.kind === "subresource") {
        ...
        if (route.subresource === "capabilities") {
          const result = referenceHostCapabilities(session.appId);
          this.auditControlRequest({
            req,
            path,
