...
static gboolean terrane_debug_build_allows_dev_flags(void) {
#ifndef NDEBUG
  return TRUE;
#else
  return FALSE;
...
static gboolean terrane_is_forbidden_dev_flag(const char *argument) {
...
static gboolean terrane_dev_flag_consumes_next_argument(const char *argument) {
  return g_strcmp0(argument, "--control-plane-port") == 0;
}

static gboolean terrane_parse_uint16(const char *text, guint *value) {
...
static gboolean terrane_parse_dev_control_options(int argc, char **argv, gboolean *enabled, guint *port, GError **error) {
  *enabled = g_strcmp0(g_getenv("TERRANE_LINUX_DEV_CONTROL"), "1") == 0;
  *port = 0;

  for (int index = 1; index < argc; index++) {
    if (g_strcmp0(argv[index], "--terrane-dev-control") == 0) {
      *enabled = TRUE;
    } else if (g_strcmp0(argv[index], "--control-plane-port") == 0) {
      if (index + 1 >= argc || !terrane_parse_uint16(argv[index + 1], port)) {
      ...
      if (!terrane_parse_uint16(value, port)) {
        g_set_error_literal(error, G_OPTION_ERROR, G_OPTION_ERROR_BAD_VALUE, "--control-plane-port requires a numeric port between 0 and 65535");
        return FALSE;
      }
    }
...
static char **terrane_application_argv_without_dev_flags(int argc, char **argv, int *application_argc) {
  ...
  for (int index = 0; index < argc; index++) {
    ...
    if (index > 0 && terrane_is_forbidden_dev_flag(argv[index])) {
      skip_next_argument = terrane_dev_flag_consumes_next_argument(argv[index]);
      continue;
    }
...
static gchar *terrane_database_path(void) {
...
static gchar *terrane_json_for_flag(const char *flag) {
...
static gchar *terrane_error_json_for_flag(const char *flag) {
  JsonBuilder *builder = json_builder_new();
  json_builder_begin_object(builder);
...
static gchar *terrane_metadata_json_for_flag(const char *flag) {
  JsonBuilder *builder = json_builder_new();
  json_builder_begin_object(builder);
...
static void terrane_bind_text(sqlite3_stmt *statement, int index, const gchar *value) {
  sqlite3_bind_text(statement, index, value, -1, SQLITE_TRANSIENT);
}

static void terrane_record_production_guard_audit(const char *flag) {
  g_autofree gchar *db_path = terrane_database_path();
  ...
  g_autofree gchar *args_json = terrane_json_for_flag(flag);
  g_autofree gchar *error_json = terrane_error_json_for_flag(flag);
  g_autofree gchar *metadata_json = terrane_metadata_json_for_flag(flag);
  ...
  if (sqlite3_prepare_v2(
          ...
          &statement,
          NULL) == SQLITE_OK) {
    terrane_bind_text(statement, 1, session_id);
    terrane_bind_text(statement, 2, metadata_json);
  ...
  if (sqlite3_prepare_v2(
          ...
          &statement,
          NULL) == SQLITE_OK) {
    terrane_bind_text(statement, 1, command_id);
    terrane_bind_text(statement, 2, session_id);
    terrane_bind_text(statement, 3, args_json);
    terrane_bind_text(statement, 4, error_json);
    sqlite3_step(statement);
  }
  sqlite3_finalize(statement);
...
static gboolean terrane_reject_dev_only_flags_if_needed(int argc, char **argv) {
  if (terrane_debug_build_allows_dev_flags()) {
    return FALSE;
  }
  for (int index = 1; index < argc; index++) {
    if (terrane_is_forbidden_dev_flag(argv[index])) {
      terrane_record_production_guard_audit(argv[index]);
...
static void on_activate(GtkApplication *application, gpointer user_data) {
  ...
  g_object_set_data_full(G_OBJECT(application), "terrane-host", host, (GDestroyNotify)webkit_host_free);
}

int main(int argc, char **argv) {
  if (terrane_reject_dev_only_flags_if_needed(argc, argv)) {
    return 1;
  }
  ...
  if (!terrane_parse_dev_control_options(argc, argv, &dev_control_enabled, &dev_control_port, &dev_control_error)) {
  ...
  GtkApplication *application = gtk_application_new("dev.terrane.webappplatform", G_APPLICATION_DEFAULT_FLAGS);
  g_signal_connect(application, "activate", G_CALLBACK(on_activate), dev_control);
  int application_argc = 0;
  char **application_argv = terrane_application_argv_without_dev_flags(argc, argv, &application_argc);
