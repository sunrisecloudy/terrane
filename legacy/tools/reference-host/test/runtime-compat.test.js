...
test("runtimeVersion mismatch quarantines the new install and leaves previous active", async () => {
  const host = new ReferenceHost({ runtimeVersion: "0.1.0" });
...
test("mount gate rejects an active incompatible install unless dev override is set", async () => {
  const strictHost = new ReferenceHost({ runtimeVersion: "0.1.0" });
  const devHost = new ReferenceHost({ runtimeVersion: "0.1.0", allowRuntimeMismatch: true });
...
test("missing optional capabilities mount but fail when called", async () => {
  const host = new ReferenceHost({ capabilityOverrides: { "dialog.saveFile": false } });
