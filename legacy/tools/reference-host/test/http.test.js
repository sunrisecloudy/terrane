...
import { startReferenceHost } from "../src/server.js";

test("http health and token-protected control command work", async () => {
  const started = await startReferenceHost({ port: 0, controlToken: "test-token" });
  try {
    ...
    assert.match(runtimeHtml, /Terrane/);
    ...
    assert.equal(bridgeCapabilities.result.platform, "reference");
    assert.equal(bridgeCapabilities.result.target, "reference-host");
    ...
    assert.equal(capabilities.result.platform, "reference");
    assert.equal(capabilities.result.target, "reference-host");
...
test("reference host writes a per-launch control token file", async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "reference-host-token-"));
  const tokenFile = path.join(dir, "control.token");
  const started = await startReferenceHost({ port: 0, tokenFile });
  try {
...
test("seeded bundled apps can use bridge permissions through HTTP", async () => {
  const started = await startReferenceHost({ port: 0, controlToken: "test-token", seedBundled: true });
  try {
