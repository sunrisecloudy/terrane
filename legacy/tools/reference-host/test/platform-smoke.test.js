...
test("checked-in platform smoke suite runs against reference-host", async () => {
  ...
  try {
    ...
    assert.equal(persisted.some((row) => row.micro_test_id === "platform-smoke:all-example-apps-cross-platform-smoke:reference-host"), true);
  } finally {
    host.close();
  }
});
