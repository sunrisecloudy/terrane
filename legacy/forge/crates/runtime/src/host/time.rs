...
//!
//! `ctx.time.now()` and `ctx.random.next()` are policy-checked against explicit
//! Time/Random manifest grants, counted against `max_host_calls`, and served by
//! the recorder's seeded logical clock / RNG (prd-merged/01 CR-11). They funnel
//! through the same [`HostContext::check_or_record_denial`] chokepoint as every
//! other host call.
...
impl HostContext<'_> {
    ...
    /// `ctx.time.now()` — checked against the manifest's Time grant, counted
    /// against `max_host_calls`, served by the seeded logical clock.
