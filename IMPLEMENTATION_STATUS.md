...
| `server/` | partial | Minimal Zig HTTP server with `/health`, `/core/step`, `/bridge` for core/capabilities/storage/mock-dialogs/notification/mock-network/app.log, `/webapps/validate`, `/webapps/install`, `/packages/*`, `/apps/{appId}/rollback`, `/webapps/examples`, `/control/command`, and token-gated safe `/db/*` inspection endpoints; uses Zig core FFI, writes a per-launch control-token file in dev mode, initializes the v0.4 logical SQLite schema, signs inline packages with Ed25519, gates installs and bridge dispatch with runtimeVersion compatibility checks, gates installs with static bundled smoke checks, enforces active-install bridge permissions, enforces active manifest `resourceBudget` limits for bridge calls/network requests/log lines/storage bytes, enforces `networkPolicy` before mock-backed network responses, serves file dialogs from `dialog_mocks`, installs packages transactionally with packaged migration chains, supports data-version-aware package rollback with snapshot data restore, creates/restores runtime snapshots, runs migration dry-run/apply controls, exposes server-side runtime open/reset/storage/log/resource control tools plus bridge-call/core-step/core-replay/assertion controls, disables dev/control endpoints in production mode, persists bridge/core/test logs, records debug bundle exports, audits accepted/rejected control requests, and temporarily bans repeated control auth failures |
