...
| `native/ios/` | partial | SwiftPM/UIKit/WKWebView scaffold with app-runtime custom-scheme resource loading, WKScriptMessageHandlerWithReply, runtime bridge envelope handling with asynchronous replies, SQLite-backed storage, native open/save file dialogs, manifest-gated network requests, native permission checks, `core.step` through a linked-or-dlopen-backed Zig C shim when Zig core symbols or `libzig_core.dylib` are available, debug-only simulator build/package/runtime-load launch smoke coverage when Xcode and CoreSimulator are available, and structured unsupported responses for unfinished platform services |
