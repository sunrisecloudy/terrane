...
| `native/macos/` | partial | SwiftPM AppKit/WKWebView host scaffold with explicit AppKit entry point, app-runtime custom-scheme resource loading, runtime bridge envelope handling, SQLite storage, open/save dialogs, manifest-gated network requests, toast logging, structured bridge responses, `core.step` through a dlopen-backed Zig C shim when `libzig_core.dylib` is available, and SwiftPM tests covering runtime resource loading, bridge dispatch, dialog open/save/cancel responses, storage persistence, Zig-backed `core.step` with a temporary dylib when Zig is available, and debug app launch smoke |
