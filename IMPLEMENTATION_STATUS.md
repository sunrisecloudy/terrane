...
| `native/macos/` | partial | SwiftPM AppKit/WKWebView host scaffold with app-runtime custom-scheme resource loading, runtime bridge envelope handling, SQLite storage, dialogs, manifest-gated network requests, toast logging, structured bridge responses, `core.step` through a dlopen-backed Zig C shim when `libzig_core.dylib` is available, and SwiftPM tests covering runtime resource loading, bridge dispatch, and storage persistence |
