...
| `server/` | partial | Minimal Zig HTTP server with `/health`, `/core/step`, `/bridge` for core/capabilities/storage/app.log, `/webapps/validate`, `/webapps/install`, `/packages/*`, `/webapps/examples`, `/control/command`, and token-gated safe `/db/*` inspection endpoints; uses Zig core FFI, initializes the v0.4 logical SQLite schema, signs inline packages with Ed25519, gates installs with static bundled smoke checks, installs packages transactionally, persists bridge/core/test logs, records debug bundle exports, and audits accepted/rejected control requests |
