...
| Path | Status | Notes |
|---|---|---|
| `runtime-web/` | partial | Launcher HTML/CSS/JS exists; fake host serves it and proxies sandbox `AppRuntime.call` messages to `/bridge` or WebKit/Android native bridge with `AppRuntime.on` events, per-mount nonce/port binding, permission, storage-prefix, network-policy, and rate-budget preflight |
