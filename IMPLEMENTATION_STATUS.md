...
| Path | Status | Notes |
|---|---|---|
| `runtime-web/` | partial | Launcher HTML/CSS/JS exists; fake host serves it and proxies sandbox `AppRuntime.call` messages to `/bridge` or WebKit native bridge with per-mount nonce/port binding plus permission, storage-prefix, network-policy, and rate-budget preflight |
