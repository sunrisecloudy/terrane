...
| `native/macos/` | partial | SwiftPM AppKit/WKWebView host scaffold with explicit AppKit entry point, app-runtime custom-scheme resource loading, runtime bridge envelope handling, SQLite storage, SQLite app-version registry rollback that restores the previous active install while preserving generated app storage, open/save dialogs, manifest-gated network requests, toast logging, structured bridge responses, `core.step` through a dlopen-backed Zig C shim when `libzig_core.dylib` is available, and SwiftPM tests covering runtime resource loading, bridge dispatch, dialog open/save/cancel responses, storage persistence, rollback, Zig-backed `core.step` with a temporary dylib when Zig is available, and debug app launch smoke |
| `native/android/` | partial | Kotlin Android scaffold with generated shared runtime/example assets, WebViewAssetLoader, WebViewCompat runtime envelope handling with origin/frame checks and asynchronous replies, SQLite-backed storage, native open/save file dialogs, manifest-derived native context, manifest-gated network requests, native permission checks, Gradle-packaged Android ABI `libzig_core.so` artifacts plus `core.step` through a JNI wrapper that loads them, structured unsupported responses for unfinished platform services, and debug APK/JNI/resource/Zig-core packaging build-smoke coverage when Gradle, Zig, and the Android SDK are available |
