...
| `server/` | partial | Minimal Zig HTTP server with `/health`, `/core/step`, `/bridge` for core/capabilities/storage/notification/app.log, `/webapps/validate`, `/webapps/install`, `/packages/*`, `/apps/{appId}/rollback`, `/webapps/examples`, `/control/command`, and token-gated safe `/db/*` inspection endpoints; uses Zig core FFI, initializes the v0.4 logical SQLite schema, signs inline packages with Ed25519, gates installs with static bundled smoke checks, enforces active-install bridge permissions, installs packages transactionally with packaged migration chains, supports data-version-aware package rollback with snapshot data restore, creates/restores runtime snapshots, runs migration dry-run/apply controls, disables dev/control endpoints in production mode, persists bridge/core/test logs, records debug bundle exports, and audits accepted/rejected control requests |
