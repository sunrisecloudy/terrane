...
| Task Workbench | `webapps/examples/task-workbench` | Stateful task workflow backed by Zig `core.step` actions | `core.step`, `storage.read`, `storage.write` | `notification.toast`, `app.log` | Add task, `core.step`, `storage.set`, toast |
