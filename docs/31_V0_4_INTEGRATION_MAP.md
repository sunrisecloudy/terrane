...
| Package validation | `docs/04_WEBAPP_PACKAGE_SPEC.md` | runtime tools/reference-host | mutation tests |
...
| Snapshot/replay | `docs/21_SNAPSHOT_AND_REPLAY_FORMAT.md` | reference-host/dev hosts | replay tests |
...
| Codex repair | `docs/25_CODEX_REPAIR_LOOP.md` | MCP + reference-host | micro-tests |
| Database schema | `docs/27_DATABASE_SCHEMA.md` + `db/` | native hosts/server/reference-host | DB tests |
...
```text
schemas
  -> reference-host package validator
  -> reference-host bridge/runtime mock
  -> Zig core stub
...
Do not start with native hosts before reference-host behavior is stable. The reference-host defines the reference contract.
...
3. all example apps run on reference-host;
