...
- [x] Source/static and simulator build checks verify iOS DEBUG simulator dev control exports/imports portable `db.export_backup` / `db.import_backup` documents over fixed app/package/storage tables through command and `/db/*` routes and records export/import rows in `backup_exports`.
...
- [x] Source/static and simulator build checks verify iOS DEBUG simulator dev control supports DB-backed one-shot `runtime.fault_inject` consumed before permission/budget bridge dispatch plus DB-backed `runtime.network_mock_set` / `runtime.network_mock_reset` and `runtime.dialog_mock_set` consumed by iOS network/dialog bridge calls.
...
- [x] Source/static checks verify Windows debug dev control routes `platform.list_webapp_versions`, `platform.install_report`, `platform.rollback_webapp`, `platform.quarantine_webapp`, `platform.uninstall_webapp`, and `platform.approve_webapp_update` through token-gated session commands with fixed SQLite app-registry helpers while Windows runtime execution remains unchecked.
- [x] Source/static checks verify Windows debug dev control routes `platform.migration_dry_run` and `platform.migration_apply` through token-gated session commands with fixed SQLite migration/storage/snapshot helpers while Windows runtime execution remains unchecked.
...
- [x] Debug dev control plane Docker-smoke verifies checked-in static `runtime.run_microtest` (`tests/micro/notes-lite-create-note.microtest.json`) and `platform.run_platform_smoke` (`tests/platform-smoke/all-example-apps.platform-smoke.json`) through repo-path-guarded command routes with persisted `micro_tests` / `test_runs` evidence.
