...
fn publish_install_records_tofu_provenance_and_replays() {
    ...
    let mut core =
        Core::open_with(dir.path().join("log.bin"), PublishEdge { pubkey: PUBKEY }).unwrap();
    ...
    assert_eq!(
        core.state().publish.provenance["demo"].publisher_pubkey,
        PUBKEY
    );
...
fn publish_fold_drops_app_provenance_but_keeps_trust() {
    let dir = tempdir().unwrap();
    let mut core =
        Core::open_with(dir.path().join("log.bin"), PublishEdge { pubkey: PUBKEY_2 }).unwrap();
