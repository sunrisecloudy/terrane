...
pub fn mcp_tools() -> Vec<ToolDef> {
    vec![
        ...
        ToolDef {
            ...
            description: "Step 2 of 4: replace one file in the draft with COMPLETE content, one file per call. Backend contract: main.js is one plain script (no top-level import/export, no modules); define one global function handle(input) where input is an array of strings and input[0] is the verb; return a string. UI contract: browser code calls window.terrane.invoke(\"verb\",\"arg1\",\"arg2\") with positional string args.",
