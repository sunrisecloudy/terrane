...
fn decide_upsert_commits_only_kv_events() {
    ...
    let decision = SearchCapability
        .decide(
            ctx(core.state()),
            "search.upsert",
            &["notes".into(), "doc-1".into(), "hello world".into()],
        )
        .unwrap();
...
fn read_resource_status_reports_document_count() {
    ...
    let ReadValue::OptString(Some(raw)) =
        SearchCapability.read_resource(ctx, "status", &[]).unwrap()
    else {
...
fn remove_deletes_doc_and_embedding_keys() {
    ...
    assert!(remaining.is_none_or(|app_kv| app_kv.keys().all(|k| !k.contains("doc-1"))));
}
