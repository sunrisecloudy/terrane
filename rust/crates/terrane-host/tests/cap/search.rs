...
"#,
    ...
    let (ok, _, err) = terrane(
        home,
        &["search", "upsert", "notes", "doc-2", "lazy dog sleeps all day"],
    );
...
fn search_notes_app_bundle_smoke() {
    ...
    let (ok, _, err) = terrane(
        home,
        &["app", "add", "search-notes", "Search Notes", "--source", &src],
    );
    assert!(ok, "app add failed: {err}");
    let (ok, _, err) = terrane(
        home,
        &["auth", "grant", "user:local-owner", "search-notes", "search"],
    );
    assert!(ok, "grant search failed: {err}");
    ...
    let (ok, out, err) = terrane(
        home,
        &["js-runtime", "run", "search-notes", "bm25", "fox"],
    );
